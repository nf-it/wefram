from typing import *
from system.requests import Request, JSONResponse, NoContentResponse
from .const import PERMISSION_ADMINISTERING
...
async def get_settings_schema(request: Request) -> JSONResponse:
    def _ordered_entities(_tab_id: str) -> List[settings.SettingsEntity]:
        _entities: Dict[Union[str, None], settings.SettingsEntity] = schema[_tab_id]
        _result: List[settings.SettingsEntity] = []
    ...
    schema: Dict[str, Dict[Union[str, None], settings.SettingsEntity]] = {}

    entity: settings.SettingsEntity
    tabs: Dict[str, settings.SettingsEntity] = {}
...
@aaa.requires(aaa.PERMISSION_ADMINUSERSROLES)
