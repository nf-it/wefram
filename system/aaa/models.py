...
class User(ds.Model):
    ...
    async def all_permissions(self) -> IPermissions:
    ...
    @classmethod
    async def all_permissions_for(cls, user_id: str) -> IPermissions:
...
class Session:
    ...
    async def create(cls, user: User) -> 'Session':
        session = cls(
            token=random_token(),
            user=user.as_dict(set_name='session'),
