...
from ..aaa import permitted, IPermissions
...
class ISitemapResponseItem(TypedDict):
    name: str
    caption: str
    order: int
    screen: Optional[str]
    icon: Optional[str]
    children: Optional[List['ISitemapResponseItem']]


ISitemapResponse = List[ISitemapResponseItem]
...
class Item:
    ...
    def as_json(self) -> ISitemapResponseItem:
...
def _make_requires(scopes: Optional[IPermissions] = None) -> IPermissions:
    if not scopes:
        return []
    return [
...
def register(screen: screens.BaseScreen) -> screens.BaseScreen:
    ...
    def decorator() -> screens.BaseScreen:
